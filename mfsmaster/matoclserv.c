...
//      if (FD_ISSET(lsock,rset)) {
                ...
                if (ns<0) {
                        ...
                        // Check if we're the leader before accepting client connections
                        if (ha_is_leader()) {
                                tcpnonblock(ns);
                                tcpnodelay(ns);
                                ...
                                // If we're not the leader, redirect client to the current leader
                                ha_master_info_t* leader = ha_get_leader();
                                if (leader) {
                                        char redirect[256];
                                        snprintf(redirect, sizeof(redirect), "REDIRECT %s:%u\n", leader->hostname, leader->port);
                                        write(ns, redirect, strlen(redirect));
                                }
                                tcpclose(ns);
                        }
                        eptr->lastwrite = now;

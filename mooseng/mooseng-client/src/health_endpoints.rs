//! Health check HTTP endpoints for MooseNG client
//! 
...
use crate::health_integration::ClientHealthService;

/// Client health endpoint service
pub struct ClientHealthEndpoints {
    health_service: Arc<ClientHealthService>,
}

impl ClientHealthEndpoints {
    pub fn new(health_service: Arc<ClientHealthService>) -> Self {
        Self { health_service }
    }
    ...
    pub async fn start_server(&self, bind_addr: &str) -> Result<()> {
        ...
        info!("Starting client health endpoint server on {}", bind_addr);
...
async fn health_status(
    State(health_service): State<Arc<ClientHealthService>>,
...
<html>
<head>
    <title>MooseNG Client Health</title>
    <style>
...
<body>
    <h1>MooseNG Client Health Dashboard</h1>
    ...
    <div class="endpoint">
        ...
        <div class="code">curl http://localhost:8080/health/detailed</div>
    </div>
    
    <div class="endpoint">
        <h3>GET /metrics</h3>
        <p>Prometheus metrics format</p>
        <div class="code">curl http://localhost:8080/metrics</div>
    </div>
    
    <footer style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #ccc; color: #666;">
        <small>MooseNG Client Health Monitor</small>
    </footer>
</body>
</html>
    "#)
